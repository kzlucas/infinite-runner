...
Le pattern de conception [`Singleton.Model<T>`](Assets/Components/Singleton/Singleton.cs) a été utilisé dans plusieurs classes du projet pour garantir qu'une seule instance de ces classes existe à tout moment pendant l'exécution du jeu.
...
- Système de Composants Modulaire
  - Chaque composant a ses propres dossiers Scripts, Prefabs et Data le cas échéant
...
- Architecture Basée sur les Événements
  - `EventBus` pour une communication découplée
  - Les composants peuvent communiquer sans références directes

- Modèle `Singleton` - Pour les gestionnaires et services globaux
- Modèle État - Implémentation explicite de `StateMachine`
- Modèle `Repository` - DataServices pour la persistance
...
L'initialisation de la scène se fait grâce aux scripts [`SceneLoader`](Assets/Components/SceneCore/SceneLoader.cs) et [`SceneInitializer`](Assets/Components/SceneCore/SceneInitializer.cs) attaché à un GameObject [Game Core](Assets/Components/SceneCore/Prefabs/SceneCore.prefab) présent dans chaque scène du jeu.
